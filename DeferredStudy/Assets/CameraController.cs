...
public class CameraController : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        tempEulerX = (int)cameraHandle.transform.eulerAngles.x;          // 拿到 cameraHandle 的默认角度
    ...
    private void SetCamPos()    // 设置camera位置
    {
        transform.localPosition = new Vector3(0, 0, -cameraZOffset); // 自动调整 cameraPos.z位置
    ...
    private void SetCamQuaternion()
    {
        ...
        model.transform.eulerAngles = tempModelEuler;  // 设置回去，这样模型的就不会跟着转了

        var lookRo = Quaternion.LookRotation(cameraHandle.transform.position - characterCamera.transform.position, Vector3.up);
        characterCamera.transform.rotation = Quaternion.RotateTowards(characterCamera.transform.rotation, lookRo, sAngle < angle ? sAngle : angle);
    }
}
