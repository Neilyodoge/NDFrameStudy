...
[ExecuteAlways]
    ...
    [System.Serializable]
        public class PlanarReflectionSettings   // 设置的内容
        {
            public ResolutionMulltiplier m_ResolutionMultiplier = ResolutionMulltiplier.Half;   // 反射精度
            public float m_ClipPlaneOffset = 0.07f;                                             // 偏移量
            public LayerMask m_ReflectLayers = -1;                                              // Layer
            public bool m_shadows;                                                              // Using Shadow
        }
        
        [SerializeField]
        public PlanarReflectionSettings m_settings = new PlanarReflectionSettings();            // 实例化设置
        public GameObject target;                                                               // 反射 Point
        [FormerlySerializedAs("camOffset")] public float m_planeOffset;                         // 偏移量
        
        private static Camera m_ReflectionCamera;                                               // private 反射相机
        // private Vector2Int m_TextureSize = new Vector2Int(256, 128);
        private RenderTexture m_ReflectionTexture = null;                                       // 反射 RT
        private int planarReflectionTextureID = Shader.PropertyToID("_PlanarReflectionTexture");// 反射 RT在shader变量中的ID
        // private Vector2Int m_OldReflectionTextureSize;

        private void OnEnable()
        {
            RenderPipelineManager.beginCameraRendering += ExecuteBeforeCameraRender;            // 直接在当前RenderPipline，在渲染之前加入这个相机
        }
