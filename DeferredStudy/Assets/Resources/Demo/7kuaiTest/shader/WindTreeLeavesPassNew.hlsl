...
v2f vert(appdata v)
{
    ...
    o.VertexColor.a = lerp(v.color.a,debug,(int)_DebugProp2.x);  // debug wind.
    o.ambient.rgb = 0;
    #ifdef _LIGHTPROBE
        // 计算环境光
...
LitFragmentOutput frag(v2f i)
{
    ...
    */
    ...
    float3 SHNOL = 0;
        ...
        vertexNdotL = lerp(vertexNdotL * 0.1,vertexNdotL,shadowAtten);  // 0.1是因为暗部本来不需要有nol但为了有点体积感，加了个0.1的nol
        SHNOL = i.ambient.rgb + vertexNdotL* _MainLightColor.rgb;
    #endif
    ...
    float GrayPart = saturate(lerp(1, i.ambient.a, /*_FaceLightGrayScale*/7));  // 亮部灰阶
    ...
    // Debug 用
    f_finalColor.rgb = lerp(f_finalColor.rgb,tintColor.rgb,_DebugProp1.x);
    f_finalColor.rgb = lerp(f_finalColor.rgb,ao.rrr,_DebugProp1.z);
    f_finalColor.rgb = lerp(f_finalColor.rgb,darkPart.rrr,_DebugProp1.w);
    f_finalColor.rgb = lerp(f_finalColor.rgb,i.VertexColor.aaa,_DebugProp2.x);
    f_finalColor.rgb = lerp(f_finalColor.rgb,subSurfaceTerm,_DebugProp2.y);
    f_finalColor.rgb = lerp(f_finalColor.rgb,lerp(0,i.ambient.rgb/2 * _refIntensity,refPart),_DebugProp2.z);
    f_finalColor.rgb = lerp(f_finalColor.rgb,darkPart.rrr,_DebugProp2.w);
...
{
    //_FlatClip = lerp(_FlatClip,1,(int)_IsPlant);
    float4 col = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, i.uv);
    float3 viewDir = normalize(GetCameraPositionWS() - i.positionWS.xyz);
    float clipPart = saturate(1 - abs(dot(i.baseNormal, viewDir)));
    // clipPart = lerp(clipPart,0.5,(int)_FlatClip);
    float dis = distance(float3(_WorldSpaceCameraPos.x, 0, _WorldSpaceCameraPos.z), float3(i.positionWS.x, 0, i.positionWS.z)/*posObj*/);
    dis = smoothstep(0, 10, dis);
    float clipValue = 0;
    //Unity_Dither(smoothstep(_DitherAmountMin, _DitherAmountMax, dis), i.positionHCS.xy, clipValue);
    Unity_Dither(dis, i.positionHCS.xy, clipValue);
    clip(min((col.a * lerp(1,_CutIntensity,_FlatClip) - clipPart), clipValue));
