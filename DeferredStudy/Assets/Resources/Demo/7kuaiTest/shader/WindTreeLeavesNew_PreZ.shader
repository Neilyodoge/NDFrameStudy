Shader "Athena/Foliage/StylizedWindTreeLeavesNew_PreZ"
{
    Properties
    {
        ...
        [Toggle]_DitherON("Dither开关",float) = 1
        [HideInInspector]_DebugProp1("",vector) = (0,0,0,0)
        [HideInInspector]_DebugProp2("",vector) = (0,0,0,0)
        ...
        [Header(Wind)]
        _Magnitude ("随风漂移强度", range(0,2)) = .5
        [HideInInspector]_Frequency ("随风飘动频率", float) = .5
        [HideInInspector]_WindSineIntensity ("风的规律波动强度", Range(0, 2)) = 0.5
        _WindDirection ("风向 (x,y,z))", Vector) = (1, 0, 1, 0)
        [HideInInspector]_WindTex ("风的波动贴图", 2D) = "black" { }
        [HideInInspector]_WindTexScale ("风的波动图的大小", float) = 1
        [HideInInspector]_WindTexIntensity ("风的波动贴图的强度", Range(0, 2)) = 1
        [HideInInspector]_WindTexMoveSpeed ("风的波动贴图的速度", float) = 0.1
